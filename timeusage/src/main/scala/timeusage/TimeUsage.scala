...
object TimeUsage extends TimeUsageInterface:
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) =
    ...
    val primaryNeeds= valueColumns.filter(x => {
      x.startsWith("t01") ||
        x.startsWith("t03") ||
        x.startsWith("t11") ||
        x.startsWith("t1801") ||
    ...
    val workingActivities = valueColumns.filter(x => {
      x.startsWith("t05") ||
        x.startsWith("t1805")
    }).map(col)
    val otherActivities = valueColumns
      .filter(x => (!primaryNeeds.contains(col(x)) && !workingActivities.contains(col(x))))
      .filter(x => {
        x.startsWith("t02") ||
          x.startsWith("t04") ||
          x.startsWith("t06") ||
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when(($"teage">=15&&$"teage"<=22), "young").when(($"teage">=23&&$"teage"<=55), "active")
